...
function ascend(builder) {
    ...
    for (blockLoc.y = Math.max(limits[0], blockLoc.y);; blockLoc.y++) {
        ...
        if (dimension.getBlock(blockLoc).isAir)
            continue;
        if (blockLoc.y + 1 <= limits[1] && !dimension.getBlock(blockLoc.offset(0, 1, 0)).isAir)
            continue;
        if (blockLoc.y + 2 <= limits[1] && !dimension.getBlock(blockLoc.offset(0, 2, 0)).isAir)
            continue;
