...
import { RawText, Vector, sleep } from "./../../../library/Minecraft.js";
...
function canSnowOn(block) {
    ...
    if (isBlocked)
        isBlocked && (isBlocked = !!dimension.getBlockFromRay(location.add([-0.49, 0, 0]), Vector.DOWN, solidTest));
    if (isBlocked)
        isBlocked && (isBlocked = !!dimension.getBlockFromRay(location.add([0.49, 0, 0]), Vector.DOWN, solidTest));
    if (isBlocked)
        isBlocked && (isBlocked = !!dimension.getBlockFromRay(location.add([0, 0, -0.49]), Vector.DOWN, solidTest));
    if (isBlocked)
        isBlocked && (isBlocked = !!dimension.getBlockFromRay(location.add([0, 0, 0.49]), Vector.DOWN, solidTest));
    return isBlocked;
}
        ...
        if (blocks.length) {
            ...
            try {
                ...
                for (let block of blocks) {
                    ...
                    while (!(block?.isValid() || (block = Jobs.loadBlock(loc))))
                        yield sleep(1);
