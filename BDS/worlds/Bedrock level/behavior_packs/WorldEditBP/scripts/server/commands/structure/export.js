...
import { RawText, regionCenter, regionIterateChunks, regionSize, Server, sleep, Vector } from "./../../../library/Minecraft.js";
...
let tempID = 0;
function writeMetaData(name, data, player) {
    if (!name.includes(":")) {
...
registerCommand(registerInformation, function* (session, builder, args) {
    ...
    let struct_name = args.get("name");
    if (!struct_name.includes(":")) {
        struct_name = "wedit:" + struct_name;
    }
    const [namespace, struct] = struct_name.split(":");
    ...
    yield* Jobs.run(session, 1, function* () {
        ...
        try {
            ...
            if (Server.runCommand(`scoreboard players set ${struct_name} wedit:exports 1`).error)
                throw "Failed to save name to exports list";
            if (excludeAir) {
                ...
                for (const [subStart, subEnd] of regionIterateChunks(...range)) {
                    while (!Jobs.loadBlock(regionCenter(subStart, subEnd)))
                        yield sleep(1);
            ...
            if (writeMetaData(namespace + ":weditstructmeta_" + struct, JSON.stringify({
                size: { x: size.x, y: size.y, z: size.z },
                relative: { x: relative.x, y: relative.y, z: relative.z },
                exporter: builder.name,
            }), builder))
                throw "Failed to save metadata";
            if (writeMetaData("weditstructref_" + struct, struct_name, builder))
                throw "Failed to save reference data";
        }
        catch (e) {
            const [namespace, name] = struct_name.split(":");
