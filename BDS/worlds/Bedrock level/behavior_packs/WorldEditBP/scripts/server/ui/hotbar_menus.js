...
HotbarUI.register("$pickPatternMask", {
        ...
        7: {
            ...
            action: (ctx, player) => {
                ...
                if (session.globalPattern.empty())
                    return ctx.error("worldedit.config.pattern.noPattern");
...
HotbarUI.register("$selectBlocks", {
    ...
    items: {
        ...
        7: {
            item: "wedit:confirm_button",
            action: (ctx, player) => {
                if (!getSession(player).selection.isValid())
                    return ctx.error("commands.generic.wedit:noSelection");
    ...
    exiting: (ctx, player) => {
        ...
        if (oldMode)
            getSession(player).selection.mode = oldMode;
    },
    cancel: (ctx) => ctx.returnto(ctx.getData("pickerData").return),
});
