...
import { contentLog, Databases, Vector } from "./../../library/Minecraft.js";
    ...
    flush() {
        for (const [chunk, data] of this.changes) {
            ...
            if (!("biomes" in database)) {
                ...
                const palette = database.data.palette;
                biomes = database.data.biomes.map((idx) => (idx ? palette[idx - 1] : -1));
            }
