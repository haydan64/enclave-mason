 ...
 * @param end
 */
export function* regionIterateBlocks(start, end, centered = false) {
 ...
 * @param end
 */
export function* regionIterateChunks(start, end, ySubChunks = true) {
...
export function regionLoaded(start, end, dimension) {
    for (const chunk of regionIterateChunks(start, end, false)) {
        if (!dimension.getBlock(chunk[0]))
            return false;
    }
    return true;
}
